...
async def _event(event):
  ...
  if row == 255:
    ...
    for k in range(row*row):
      i = int(k/row)
      j = k % row
      buttons[i][j] = Button.inline('\u2605' if (res >> i*row+j) & 1 else '\u3000', buttons[i][j].data)
    ...
    await event.edit(caption+f"当前大小: {len(buttons[0])+1}", buttons=gen_buttons(len(buttons[0])+1))
  elif row == 252:
    if len(buttons[0]) <= 1:
    ...
    for k in [ij, ij+1, ij-1, ij+row, ij-row]:
      ...
      if (
        ...
        buttons[i][j] = Button.inline('\u3000' if buttons[i][j].text == '\u2588' else '\u2588', buttons[i][j].data)
...
def gen_buttons(row):
  ...
  buttons = [
    [Button.inline('\u3000', b'light_' + row_bytes+(i*row+j).to_bytes(2, 'big')) for j in range(row)] for i in range(row)
  ]
  buttons.extend(([
